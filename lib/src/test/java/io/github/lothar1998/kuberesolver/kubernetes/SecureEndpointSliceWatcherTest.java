...
import io.github.lothar1998.kuberesolver.kubernetes.SecureEndpointSliceWatcher.AuthConfigProvider;
...
public class SecureEndpointSliceWatcherTest {
    ...
    private static final WireMockExtension WIREMOCK = WireMockExtension.newInstance()
            .options(
                    wireMockConfig()
                            .dynamicHttpsPort()
                            .httpDisabled(true)
                            .keystorePath(getResourcePath("wiremock.jks"))
                            .keystorePassword("password"))
            .build();

    private static final AuthConfigProvider AUTH_CONFIG_PROVIDER = new AuthConfigProvider() {
        @Override
        public InputStream getCaCert() throws Exception {
            return new FileInputStream(getResourcePath("wiremock.crt"));
        }
    ...
    @DisplayName("should watch on events from HTTPS server until the server ends the stream")
    @Test
    void watchAllStreamedEventsTest() {
        ...
        var watcher = new SecureEndpointSliceWatcher(WIREMOCK.getRuntimeInfo().getHttpsBaseUrl(), "my-namespace",
    ...
    void watchFailsDueToInvalidURL() {
        var watcher = new SecureEndpointSliceWatcher("invalid_path", "default", AUTH_CONFIG_PROVIDER);
        var subscriber = mock(SecureEndpointSliceWatcher.Subscriber.class);
    ...
    void watchFailsDueToInvalidURI() {
        var watcher = new SecureEndpointSliceWatcher("http://invalid path", "default", AUTH_CONFIG_PROVIDER);
        var subscriber = mock(SecureEndpointSliceWatcher.Subscriber.class);
        watcher.watch("service", subscriber);
    ...
    @Test
    void watchFailsDueToNotOkHTTPStatusCode() {
        WIREMOCK.stubFor(
                get(urlPathTemplate(PATH_TEMPLATE))
                        .withPathParam("namespace", equalTo("my-namespace"))
                        .withQueryParam("labelSelector", equalTo("kubernetes.io/service-name=my-service"))
    ...
    private static String getResourcePath(String filename) {
        return SecureEndpointSliceWatcherTest.class.getClassLoader().getResource(filename).getPath();
    }
}
